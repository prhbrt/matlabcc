...
void mexFunction(int nargout, mxArray *argout[], int nargin, const mxArray *argin[])
{
  ...
  mtb::Mat<double, false> luminance(argout[0]);
  
  argout[1] = mtb::create<3, double>(im.size(0), im.size(1), im.size(2));
  mtb::Mat<double, false> contrast(argout[1]);
  
  argout[2] = mtb::create<3, double>(im.size(0), im.size(1), im.size(2));
  mtb::Mat<double, false> structure(argout[2]);
  ...
  mtb::forEachElement<3>(im, [&](size_t row, size_t col, size_t channel) {
    ...
    luminance(row, col, channel) = sim(mean, meanr, 0.01);
    contrast (row, col, channel) = sim(var, varr , 0.03);
    structure(row, col, channel) = (cov + 0.015) / (var*varr + 0.015);
  });
}
